...
public class SwerveDrive extends Command {
  ...
  public void execute() {
    ...
    double translationVal = translationLimiter.calculate(
      MathUtil.applyDeadband(translationSupplier.getAsDouble()/1.9, Swerve.stickDeadband));
    double strafeVal = strafeLimiter.calculate(
      MathUtil.applyDeadband(strafeSupplier.getAsDouble()/1.9, Swerve.stickDeadband));
    double rotationVal = rotationLimiter.calculate(
      MathUtil.applyDeadband(rotationSupplier.getAsDouble()/1.9, Swerve.stickDeadband));
